...
@section('content')
    ...
    <form action="{{ route('perbaikan.store') }}" method="POST">
        ...
        <div class="mb-3">
            ...
            <input type="date" name="tanggal_lapor" id="tanggal_lapor" class="form-control" value="{{ old('tanggal_lapor') }}" required>
            @error('tanggal_lapor')<small class="text-danger">{{ $message }}</small>@enderror
        </div>

        <div class="mb-3">
            <label for="kerusakan" class="form-label">Kerusakan</label>
            <textarea name="kerusakan" id="kerusakan" class="form-control" rows="3" required>{{ old('kerusakan') }}</textarea>
